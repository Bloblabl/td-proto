...
const SPECS: Record<string, ShapeSpec> = {
  ...
  // Вихрь — трёхлопастная вертушка: разгон скорости атаки
  whirl: {
    hint: '🌀 разгон по цели',
    make: (s, size, c) => s.add.star(0, 0, 3, size * 0.18, size * 0.5, c)
  },
  // Пушка — восьмиугольное ядро: тяжёлый AoE
  cannon: {
    hint: '● тяжёлый AoE',
    make: (s, size, c) => s.add.polygon(0, 0,
      poly(size, [[0.41, -1], [1, -0.41], [1, 0.41], [0.41, 1], [-0.41, 1], [-1, 0.41], [-1, -0.41], [-0.41, -1]]), c)
  },
  // Ростовик — ёлочка-росток: урон растёт с волнами
  growth: {
    hint: '🌱 растёт с волнами',
    make: (s, size, c) => s.add.polygon(0, 0,
      poly(size, [[0, -1], [0.5, -0.35], [0.25, -0.35], [0.7, 0.35], [0.35, 0.35], [0.8, 1], [-0.8, 1], [-0.35, 0.35], [-0.7, 0.35], [-0.25, -0.35], [-0.5, -0.35]]), c)
  },
  // Палач — топор: казнит цели с низким HP
  exec: {
    hint: '🪓 добивает слабых',
    make: (s, size, c) => s.add.polygon(0, 0,
      poly(size, [[-0.2, -1], [0.2, -1], [0.2, -0.55], [0.95, -0.75], [1, 0], [0.95, 0.75], [0.2, 0.55], [0.2, 1], [-0.2, 1]]), c)
  },
  // Вампир — клыки: часть урона возвращает маной
  vamp: {
    hint: '🩸 урон → мана',
    make: (s, size, c) => s.add.polygon(0, 0,
      poly(size, [[-0.9, -1], [0.9, -1], [0.5, 0.3], [0.25, -0.4], [0, 0.9], [-0.25, -0.4], [-0.5, 0.3]]), c)
  },
  // Мим — аморфный блоб: сливается с любым типом того же ранга
  mimic: {
    hint: '❔ джокер мерджа',
    make: (s, size, c) => s.add.polygon(0, 0,
      poly(size, [[0, -1], [0.9, -0.2], [0.55, 0.9], [-0.65, 0.75], [-0.85, -0.35]]), c)
  }
};
  ...
  switch (typeId) {
    ...
    case 'whirl': {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        spin.push([Math.cos(a) * r, Math.sin(a) * r]);
      }
      return wrap(`<polygon points="${pts(spin)}" fill="${color}"/>`);
    }
    case 'cannon':
      return wrap(`<polygon points="${pts([[0.41, -1], [1, -0.41], [1, 0.41], [0.41, 1], [-0.41, 1], [-1, 0.41], [-1, -0.41], [-0.41, -1]])}" fill="${color}"/>`);
    case 'growth':
      return wrap(`<polygon points="${pts([[0, -1], [0.5, -0.35], [0.25, -0.35], [0.7, 0.35], [0.35, 0.35], [0.8, 1], [-0.8, 1], [-0.35, 0.35], [-0.7, 0.35], [-0.25, -0.35], [-0.5, -0.35]])}" fill="${color}"/>`);
    case 'exec':
      return wrap(`<polygon points="${pts([[-0.2, -1], [0.2, -1], [0.2, -0.55], [0.95, -0.75], [1, 0], [0.95, 0.75], [0.2, 0.55], [0.2, 1], [-0.2, 1]])}" fill="${color}"/>`);
    case 'vamp':
      return wrap(`<polygon points="${pts([[-0.9, -1], [0.9, -1], [0.5, 0.3], [0.25, -0.4], [0, 0.9], [-0.25, -0.4], [-0.5, 0.3]])}" fill="${color}"/>`);
    case 'mimic':
      return wrap(`<polygon points="${pts([[0, -1], [0.9, -0.2], [0.55, 0.9], [-0.65, 0.75], [-0.85, -0.35]])}" fill="${color}"/>`);
    default:
